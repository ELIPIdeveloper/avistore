...
window.AVISTORE = (function(){
  ...
  function fmtPrice(n){
    n = Number(n) || 0;
    var toman = n / 10; // قیمت‌ها در دیتا به ریال ذخیره شده‌اند؛ فقط در نمایش به تومان تبدیل می‌شوند
    return toman.toLocaleString("fa-IR") + " " + CONFIG.CURRENCY_LABEL;
  }
  ...
  /* ---------- صفحه‌بندی مشترک (خانه / جستجو) ----------
     current: شماره صفحهٔ جاری (از ۱)، totalPages: تعداد کل صفحات.
     یک نوار شمارهٔ صفحه می‌سازد؛ کلیک روی هر دکمه رویداد data-page دارد
     و صفحات کناره‌ای وقتی صفحات زیاد باشند با «…» جمع می‌شوند. */
  function paginationHtml(current, totalPages){
    if(totalPages <= 1) return "";
    var pages = [];
    var add = function(n){ if(pages.indexOf(n) === -1) pages.push(n); };
    add(1); add(totalPages);
    for(var i = current - 1; i <= current + 1; i++){ if(i >= 1 && i <= totalPages) add(i); }
    pages.sort(function(a,b){ return a - b; });

    var html = '<div class="pagination">';
    html += '<button type="button" class="page-btn nav" data-page="' + (current - 1) + '" ' + (current <= 1 ? "disabled" : "") + ' aria-label="صفحه قبل">‹</button>';
    var prev = 0;
    pages.forEach(function(n){
      if(prev && n - prev > 1) html += '<span class="page-dots">…</span>';
      html += '<button type="button" class="page-btn' + (n === current ? " on" : "") + '" data-page="' + n + '">' + n.toLocaleString("fa-IR") + '</button>';
      prev = n;
    });
    html += '<button type="button" class="page-btn nav" data-page="' + (current + 1) + '" ' + (current >= totalPages ? "disabled" : "") + ' aria-label="صفحه بعد">›</button>';
    html += '</div>';
    return html;
  }
  function wirePagination(el, onGoTo){
