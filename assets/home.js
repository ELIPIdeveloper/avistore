(function(){
  ...
  function renderAll(){
    ...
    allGrid.innerHTML = pageItems.map(function(p){
      return A.cardHtml(p, { showDesc: false, showAdd: false });
    }).join("");
    A.wireRevealOnce(allGrid);
    allPagination.innerHTML = A.paginationHtml(currentPage, totalPages);
  }
  A.wirePagination(allPagination, function(n){
    currentPage = n;
    renderAll();
